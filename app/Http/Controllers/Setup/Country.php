...
class Country extends Controller
{
    ...
    private $SubFolder = "/country"; //Sub Folder Name inside the resources/views/$ThemePath/pages/$MainFolder/
    private $Upload = ""; //Upload Folder Name inside the public/admin/media

    private $Access = "shipment-country"; // Access level for this controller
    private $ParentRoute = "setup-country"; // Parent Route Name Eg. vrm-settings
    ...
    private $New = 'setup-country/add'; // New
    private $Save = 'setup-country/save'; // Add New
    private $Edit = 'setup-country/edit'; // Edit
    private $Update = 'setup-country/update'; // Update
    private $Delete = 'setup-country/delete'; // Delete
    ...
    private function passedSettings($addtional_data = [])
    {
        ...
        // Shipment Status
        $setting['entry_list'] = Hierarchy::with(['attributes'])->where('type', 'country')->latest()->get();
    ...
    private function show($data, $layout = 'main')
    {
        ...
        if (\App\Models\Vrm\Auth::auth($this->Access)) {
            ...
            // error to not allowed page
            return view("vrm-admin/errors/not-allowed", $data);
        }
    }
    ...
    public function store(Request $request, $action = '')
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'country_dial_code' => "nullable|max:10",
        ]);
        ...
        $insertFrom['parent'] = $validated['parent'];
        ...
        $insertFrom['attributes'] = [
            ...
            'country_dial_code' => $validated['country_dial_code'],
        ];
        ...
        if ($saved) {
            ...
            if (isset($attributes)) {
                // HierarchyAttributes
                $meta = new HierarchyAttributes();
                foreach ($insertFrom['attributes'] as $key => $value) {
                    $meta->hierarchy_id = $saved->id;
                    $meta->name = $key;
                    $meta->value = $value;
                    $meta->save();
                }
            ...
            // Open Page
            return $this->index('<strong>Error:</strong> Entry creation was successful.');
        } else {
            // Notification
            session()->flash('notification', 'error');
    ...
    public function update(Request $request, $action = '')
    {
        ...
        $updateFrom['parent'] = $validated['parent'];
    ...
    public function delete(Request $request, $action = '')
    {
        ...
        // ? Return error
        return redirect()->back()->with('message', 'Failed to delete, please try again.');
    }
}
