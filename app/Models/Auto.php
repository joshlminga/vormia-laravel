...
class Auto extends Model
{
    ...
    {
        $data['breadcrumb'] = [];
        $data['brand_name'] = "ENTER-BUSINESS-NAME";
        $data['pagetitle'] = '';
        $data['main_website'] = '';
        $data['base_url'] = url('/');
    ...
    /**
     * Todo: Filter Main
     * ? 1: From the search results, filter the results
     * ? 2: Return the results
     *
     * @param array $hierarchies
     */
    public static function getInheritance($hierarchies)
    {
        // To Assign
        $found = null;
        ...
        foreach ($hierarchies as $hierarchy) {
            if ($hierarchy->type == 'country' && $hierarchy->parent == 0) {
                $found['countries'][] = $hierarchy;
            } elseif ($hierarchy->type == 'city' && $hierarchy->parent != 0) {
                // Get Slug for Category
                $hierarchy->parent_slug = Term::getSlug(['table' => 'hierarchies', 'related_id' => $hierarchy->parent]);
                $found['cities'][] = $hierarchy;
            }
        }
    ...
    public static function uploadImage(array $images, string $upload_path = null, bool $year_folder = true, bool $randomize_file_name = true, bool $private_upload = false, string $convert = 'webp')
    {
        ...
        foreach ($uploaded as $img) {
            ...
            $image_thumb = ImageControl::thumbnailImage($img, width: 79, height: 79, override: true);
            // resize image
            $image_resized = ImageControl::resizeImage($img, width: 471, height: 550, add_background: true);
